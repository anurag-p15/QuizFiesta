...
function Personal_Quiz() {
  ...
  const handleQuizCodeSubmit = async (event) => {
    ...
    try {
      const response = await axios.get('http://localhost:5000/api/quiz_search/personal_quiz', {
      ...
      if (response.data.message === 'Code Found') {
        ...
        try {
          const questionsResponse = await axios.get('http://localhost:5000/api/quiz_questions/personal_quiz', {
            params: { code: quizCode }
          });
  ...
  const sendQuizCompletionData = async () => {
    try {
      const response = await axios.post('http://localhost:5000/api/test_score/personal_quiz', {
        score,
